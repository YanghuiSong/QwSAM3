...
When we're looking for the row EDT at location j, we're effectively looking for min_i (x-i)^2 + F(i). In other word we want to find the lowest parabola at j.
...
One could emulate these operations with masking, but in initial tests, it proved to be worse than naively reading and writing to the global memory. My guess is that the cache is compensating somewhat for the repeated single-point accesses.
...
def edt_triton(data: torch.Tensor):
    ...
    if not TRITON_AVAILABLE:
        # Fallback implementation using CPU-based OpenCV
        return edt_fallback(data)
...
def edt_fallback(data: torch.Tensor):
    """
    Fallback implementation of EDT using CPU-based OpenCV.
    This is used when Triton is not available (e.g., on Windows).
    """
    import cv2
    import numpy as np
    ...
    for b in range(data_np.shape[0]):
        ...
        # Note: OpenCV computes distance to nearest 0, so we invert the mask
        inv_mask = 1 - mask
        dist_transform = cv2.distanceTransform(inv_mask, cv2.DIST_L2, 5)
        
        result_np[b] = dist_transform
    
    # Return to original device and dtype
    return torch.from_numpy(result_np).to(device=data.device, dtype=data.dtype)